...
x = mostOftenWinner;
finalTraits(x).honesty = traits(x).honesty;
finalTraits(x).trust = traits(x).trust;
finalTraits(x).aggressive = traits(x).aggressive;
finalTraits(x).threshold = traits(x).threshold;
finalTraits(x).shifty = traits(x).shifty;
finalTraits(x).consideration = traits(x).consideration;
