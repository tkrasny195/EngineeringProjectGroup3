...
clc
n = 10; 
zeta = 100; % the number of games in each level of dogfight between the winners
weightOfConsistency = 5;
control = 2.1;
switch control
    case 1
        tic
        [traits, winnersArray] = ldGame(n,weightOfConsistency);
        toc
        levels = false;
    case 2
        tic
   ...
   case 2.1
        tic
        [finalTraits,winnersDogfight,mostOftenWinner,bunchOfTraits,bestPlayerNumber] = DifferentdogfightNI(zeta);
        toc
        levels = true;
        level = 2;
    case 3
...
if control == 2.1
    disp(['Best player should be: ' num2str(bestPlayerNumber)])
    disp(['Most often winner was: ' num2str(mostOftenWinner)])
end

if control == 2.1
    for i = 1:6
        [string3,~,percentageWins] = calcWins(winnersDogfight,i);
        disp(['Player ' num2str(i) '''s ' string3]);
        bunchOfTraits(i).percentage = percentageWins;
        bunchOfTraits(i).identifier = 0;
        bunchOfTraits(i).numberOfGames = zeta;
    end
    
    bunchOfTraits(bestPlayerNumber).identifier = 'Best Player';
    bunchOfTraits(mostOftenWinner).identifier = 'Won the Most';
end
